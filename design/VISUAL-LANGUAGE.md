...
Scroll-linked, never scroll-jacked. The page never takes the scroll, never snaps it, and never plays motion the hand did not make. A held scene comes from `position: sticky` and releases the moment the reader keeps walking. Under `prefers-reduced-motion: reduce` both classes arrive fully drawn and nothing moves. The evidence section refuses motion entirely, in every mode: absence does not perform.
