...
8. A `supports-tested-context` result for the primary hypothesis requires a complete run, a performed human Attend wherever the scope requires it, a predeclared action whose observed follow-up improved on its baseline, a completed observation window, and an observed baseline and follow-up for every affected party's exposure, extractability, and shifted burden, with the follow-up settling the material-increase question. An estimate does not settle it, and a party who turned out to carry no exposure is recorded as observed with no material increase rather than left unobserved. A window closed as refused or unmeasurable produced no evidence and carries nothing. Unknown harm evidence does not show that no material increase occurred, and an unread baseline leaves the follow-up nothing to be an increase over. A partial run keeps the narrower classification its own design or secondary hypothesis earns. `support_blockers()` in [scripts/check_repo.py](scripts/check_repo.py) enforces the rule and names each missing piece. Nothing checks this paragraph against that function, so a change to one is owed to the other.
