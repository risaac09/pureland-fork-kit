...
| Material provided | The public repository; a private synthesis note from the maintainer's own research vault (treated as a lead, not a source, since its own citation list does not resolve); a private register-discipline standard (`buddhist-flattening-critique.md`) the maintainer already holds elsewhere in his stack |
...
| Human review | Maintainer review remains required before merge. The mark options, the practice-frame reorganization question, and a referenced "rain metaphor" are named as open questions in the pull request rather than resolved here |
